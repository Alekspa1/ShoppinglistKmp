    ...
    { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            ShopingList(
                ...
                itemList,
                onShowDialogChange = { newId,state,isWho ->
                    id = newId
                    who = isWho
                    showDialog = state
                },
                onDeleteButtonOrChangeItem = { id, action->
                    when(action){
                            ITEM,LIST-> viewModel.deleteFun(id,action)
                            CROSS_IT-> viewModel.updateFun(id,"","",action)
    ...
    if (showDialog) {
        AddItemsDialog(
            ...
            onConfirm = { text, count ->
                when (who) {
                    SETTINGS -> {
                        viewModel.initKey(text)
                    }

                    ITEM -> {
                        viewModel.addItemsShopListFun(text, count.toInt())
                    }

                    LIST -> {
                        viewModel.createShopList(text)
                    }
                    UPDATES ->{
                        viewModel.updateFun(id,text,count,UPDATES)
                    }
                }
                showDialog = false
            },
